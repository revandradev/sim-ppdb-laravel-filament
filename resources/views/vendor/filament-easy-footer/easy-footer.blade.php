...
<footer @class([
    ...
    'border-t border-gray-200 dark:border-gray-700 text-center p-2' =>
        $footerPosition === 'sidebar' ||
        $footerPosition === 'sidebar.footer' ||
        $borderTopEnabled === true,
    'fi-sidebar gap-2 h-auto' =>
        $footerPosition === 'sidebar' || $footerPosition === 'sidebar.footer',
    'gap-4' =>
        $footerPosition !== 'sidebar' && $footerPosition !== 'sidebar.footer',
    'mx-auto w-full px-4 md:px-6 lg:px-8' => $footerPosition === 'footer',
    match (
        ($maxContentWidth ??=
            filament()->getMaxContentWidth() ?? Width::SevenExtraLarge)
    ) {
    ...
    @if ($logoPath)
        <span class="flex items-center gap-2">
            ...
            @endif
            <img src="{{ $logoPath }}" alt="Logo" class="w-auto object-contain"
